/* ==========================================================================
   ...
   --------------------------------------------------------------------------
   Ponto de entrada do sistema (Controller).
   Orquestra a inicialização, gerencia eventos globais e conecta
   os módulos de Lógica (Storage/Auth) com a Visualização (Render/Modals).
   ========================================================================== */
...
/**
 * Objeto central que armazena referências aos elementos do DOM.
 * Evita consultas repetitivas (document.getElementById) durante a execução.
 */
const elements = {
    // Layout Geral
 ...
 * Garante a persistência da preferência visual do usuário.
 */
...
/**
 * Alterna entre temas Claro e Escuro, salva a preferência e atualiza o ícone.
 */
...
/**
 * Atualiza o ícone do botão de tema (Sol/Lua) conforme o estado atual.
 */
...
/**
 * Configura o comportamento de esconder o Header ao rolar para baixo (UX Mobile).
 */
    ...
    window.addEventListener('scroll', () => {
        ...
        if (!ticking) {
            window.requestAnimationFrame(() => {
                // Esconde se rolar para baixo mais de 50px
 ...
 * Centraliza a lógica de interação (cliques, submits, changes).
 */
function setupEventListeners() {
    ...
    const handleAuth = (isRegister) => {
        ...
        if (isRegister) registerUser(u, p);
        else loginUser(u, p);
    };
    ...
    document.addEventListener('render-history', () => {
        import('./render.js').then(({
            renderHistoryDrawer
        }) => renderHistoryDrawer());
    });

    // --- 3.9 MÁSCARAS DE INPUT (MOEDA) ---
    const moneyInputs = [
...
function handleTransactionFormSubmit(event) {
    ...
    if (amount <= 0) {
        showToast(TEXT.amountPositive, 'warning');
        return;
    }
    if (!accountId) {
        showToast(TEXT.accountRequired, 'warning');
        return;
    }
    ...
    if (type === 'expense') {
        ...
        if (!toAccId) {
            showToast(TEXT.destinationAccountRequired, 'warning');
            return;
        }
        if (toAccId === accountId) {
            showToast(TEXT.differentAccountsRequired, 'warning');
            return;
        }
